...
\begin{abstract}
We propose a novel ray-based framework for modeling gravitational fields, 
where the field at an observation point is defined as the aggregation of 
line-integrated mass contributions along all incoming directions. In the 
static case, this formulation reproduces the classical Newtonian 
inverse-square law, demonstrating consistency with established gravitational 
theory. A preliminary exploration suggests that the framework may be 
extended to account for relativistic effects, incorporating special-relativistic 
aberration and retarded times to describe the influence of uniformly moving 
sources. This work represents an initial step in ray-based gravitational 
modeling, open to further refinement and generalization.
\end{abstract}
...
entirely within the moving frame. A worked example suggest consistency with 
the linearized Liénard--Wiechert field.
...
where $\pv \in \R^N$, $\Omega=\sphere$ is the unit angular domain, $L(\pv,\uh)$ is the ray 
$\{\,\pv+c\;t\,\uh:\ t\le 0\,\}$, $\dd\ell$ is the arc length along the ray, and
$G$ is a coupling factor.
...
$\rho_0=\frac{3M}{4\pi R^3}$. Compute the ray-based gravitational field at a point 
located at distance $D > R$ from the sphere's center.
...
$\theta_0=\pi - \arcsin(R/D)$. For such directions, the line-integrated density equals the 
chord length inside the sphere times $\rho_0$:
\[
...
By symmetry, $\gray(\pv)$ is parallel to $\hat{d}$. Using spherical coordinates around 
$\hat{d}$, $\dd\Omega=\sin\theta\,\dd\theta\,\dd\phi$, the ray-based field is
\[
\gray(\pv)
...
To demonstrate the equivalence, we will expand the line integral in the ray-based field 
definition, then we will rewrite the classical Newtonian field in spherical coordinates, 
and finally we will compare the two expressions to show they are identical.
...
In the Newtonian field, substitute $\xv = \pv - s \uh$ and $\|\pv - \xv\| = s$. Then:
\[
...
is the ray $\{\,\pv+c\;t\,\uh:\ t\le 0\,\}$, $\dd\ell$ is the arc length along the ray, 
$G$ is a coupling factor, and $h(\uh; \vv)$ is the angular aberration function 
that transforms the direction $\uh$ to the corresponding direction in the moving 
frame.
...
We resolve the same problem as in the previous example using the alternative definition of the gravitational field \eqref{eq:rayfield-rel-alt}.
...
\textbf{Waves instead of rays}. Consider a wave as the transport element. Try to
obtain the equivalence statement in the static case. Explore whether it has any 
relation to the wave-particle duality. Investigate all forms of \emph{ray/wave} 
satisfying the equivalence result.
